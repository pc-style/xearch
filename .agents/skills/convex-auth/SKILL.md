...
5. Wire the client for the selected path. For Password, magic link/OTP, or OAuth through `@convex-dev/auth`, use `ConvexAuthProvider`, the matching sign-in component, and route guards. For Passkey, use the selected integration's client provider or Convex token bridge (`ConvexProviderWithAuth` or its integration-specific wrapper when applicable), not `ConvexAuthProvider` by default. Only if the implementation needs shadcn/ui primitives, first select an approved exact shadcn CLI version, add it to devDependencies with Bun, and commit the resulting `bun.lock`. Then use `bunx --no-install shadcn add <name>` before importing the generated component. Do not install shadcn for an implementation that does not use it.
...
- Install any shadcn/ui primitive you import up front with the approved, exact-version local CLI (`bunx --no-install shadcn add ...`); do not download a mutable `latest` CLI. A missing @/components/ui/* is a hard build failure.
