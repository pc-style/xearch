...
- SELF-VERIFY RULE — use the repository's Bun toolchain and committed `bun.lock`: install with `bun install --frozen-lockfile`, then run the available `bun run typecheck`, `bun run lint`, and `bun run test` scripts and fix every reported error. Deployment verification is separate: identify and announce a valid target through deploy-guard, and require fresh consent for production. If no valid target or network exists, report that deployment verification was not performed; never force anonymous mode or invent a target.
