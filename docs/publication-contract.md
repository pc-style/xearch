...
scenario-publication-lifecycle.test.ts`, against an in-memory convex-test
  deployment. The in-repo sender has since called the **production** route
  over TLS with a deliberately unknown handle (422) and a wrong bearer (401),
  mutating no account state. No real account has been published yet.
