...
async function chunk(n, from, to) {
  ...
  for (let f = from; f < to; f++) {
    ...
    if ((f - from) % 30 === 29) {
      ...
      console.log(
        `worker ${n}: frame ${f + 1}/${to} · ${rate.toFixed(2)} fps · ${((to - f - 1) / rate / 60).toFixed(1)} min left`,
      );
    }
  }
...
async function video() {
  ...
  const files = await Promise.all(
    Array.from({ length: WORKERS }, (_, n) => chunk(n, n * per, Math.min(total, (n + 1) * per))),
  );
  ...
  run([
    ...
    wav,
    "-vf",
    "zscale=m=709:r=limited:p=709:t=709,format=yuv420p",
    "-c:v",
    "libx264",
    "-preset",
    "slow",
