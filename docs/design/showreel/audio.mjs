...
// Render a mono generator into a bus with equal-power pan, gliding from pan to panEnd.
function voice(t0, len, gen, { gain = 1, pan = 0, panEnd = pan, rev = 0, bus = main } = {}) {
...
function sub(t0, g = 1, len = 2.2, f0 = 34) {
  ...
  voice(
    ...
    (t) => {
      ...
      return (
        Math.tanh(Math.sin(ph) * 1.6) * Math.exp(-t * (2.6 / len) * 2) * Math.min(1, t / 0.003)
      );
    },
    { gain: 0.7 * g },
  );
...
function bass(t0, len, f) {
  ...
  voice(
    ...
    (t) => {
      ...
      const env =
        Math.min(1, t / 0.004) *
        (t < len ? 0.75 + 0.25 * Math.exp(-t * 18) : Math.exp(-(t - len) * 60));
  ...
  notes.forEach((n, j) => {
    ...
    voice(
      ...
      (t) => {
        ...
        return (
          lp(saw(a) + saw(b), 400 + 5200 * Math.exp(-t * 9), 1.2) *
          Math.exp(-t * 5.5) *
          Math.min(1, t / 0.002)
        );
      },
      {
        gain: (0.2 * g) / Math.sqrt(notes.length),
        pan: (j / (notes.length - 1 || 1) - 0.5) * 0.9,
        rev: 0.4,
      },
    );
  });
...
function pluck(t0, n, g = 1, pan = 0, decay = 7) {
  ...
  voice(
    ...
    (t) => {
      ...
      return (
        Math.sin(ph + 1.2 * Math.exp(-t * 14) * Math.sin(ph * 2)) *
        Math.exp(-t * decay) *
        Math.min(1, t / 0.001)
      );
    },
...
function crash(t0, g = 1, len = 2.5) {
  for (const pan of [-0.5, 0.5]) {
    ...
    voice(
      t0,
      len,
      (t) => hp(white(), 4200, 0.6) * Math.exp(-t * (5 / len)) * Math.min(1, t / 0.002),
      { gain: 0.14 * g, pan, rev: 0.25 },
    );
  }
}
...
function whoosh(t0, len, f0, f1, g = 1, p0 = 0, p1 = 0) {
  ...
  voice(
    ...
    { gain: 0.3 * g, pan: p0, panEnd: p1, rev: 0.18 },
  );
}
...
function sweep(t0, len, f0, f1, g = 1, p0 = 0, p1 = 0) {
  ...
  voice(
    ...
    { gain: 0.07 * g, pan: p0, panEnd: p1, rev: 0.3 },
  );
}
  ...
  voice(
    ...
    (t) => {
      ...
      parts.forEach(([r, a, d], i) => {
        ph[i] += (TAU * f * r) / SR;
        x += Math.sin(ph[i]) * a * Math.exp(-t * d);
      });
...
for (let bar = 2; bar <= 6; bar++)
  pad(bt(bar * 4), bt(bar === 6 ? 3 : 4), chords[bar], 1, 0.25, 0.4);
...
for (let k = 0; k < 20; k++)
  tick(bt(10.5) + (k * B) / 8, 0.9, 2000 + ((k * 7) % 5) * 180, -0.2 + (k / 20) * 0.4);

thock(bt(13), 1);

bell(bt(13) + 0.01, 89, 0.35);

[65, 68, 70, 72, 75].forEach((n, i) =>
  pluck(bt(13.45) + (i * B) / 4, n + 12, 0.7, -0.3 + i * 0.15),
);
...
const io = (() => {
  // same cubic-bezier(.65,0,.35,1) the scan uses
...
for (const [n, d] of [
  ...
  voice(
    ...
    (t) => {
      ...
      return (
        Math.sin(ph) *
        (0.6 + 0.4 * Math.sin(t * TAU * 12)) *
        Math.sin((Math.PI * t) / (bt(23.4) - bt(22)))
      );
    },
    { gain: 0.022, rev: 0.5, pan: 0.4 },
  );
}
...
// 6 · wall
{
  ...
  voice(
    ...
    bt(1),
    (t) => {
      ph += (TAU * expLerp(90, 900, t / bt(1))) / SR;

      return saw(ph / TAU) * Math.pow(t / bt(1), 2);
    },
    { gain: 0.05, rev: 0.2 },
  );
}
...

{
  ...
  voice(
    ...
    (t) => {
      ...
      return lp(white(), expLerp(200, 12000, u), 0.8) * Math.pow(u, 3);
    },
    { gain: 0.5, rev: 0.1 },
  );
}
...
function freeverb(inp) {
  ...
  const combs = [1116, 1188, 1277, 1356, 1422, 1491, 1557, 1617].map((d) => ({
    buf: new Float32Array(Math.round(d * s)),
    i: 0,
    f: 0,
  }));
...
for (let i = 0; i < N; i++)
  for (let c = 0; c < 2; c++)
    wav.writeInt16LE(Math.round(clamp(out[c][i] * norm, -1, 1) * 32767), 44 + i * 4 + c * 2);
